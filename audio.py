...
def _get_silero_model():
    ...
    try:
        ...
        logger.warning(f"  ⚠️   [VAD] Could not load Silero VAD ({exc}). Falling back to energy VAD.")
        return None, None
...
def record(output_path: str = config.RECORDED_WAV) -> str:
    ...
    """
    logger.debug(f"  🎙  Listening for {config.RECORD_SECONDS} seconds...")
...
def record_vad(output_path: str = config.RECORDED_WAV) -> str:
    ...
    with sd.InputStream(
        ...
        while True:
            ...
            # ── Per-chunk diagnostic (DEBUG only) ───────────────────────────
            if logger.is_debug():
            ...
            if total_samples >= max_samples:
                stop_reason = "max_duration"
                logger.warning(f"  ⚠️   [VAD] Max duration ({config.VAD_MAX_DURATION}s) reached — stopping.")
                break

    frames   = np.concatenate(all_chunks, axis=0)
    duration = total_samples / config.SAMPLE_RATE
    logger.debug(f"  ✅  Recorded {duration:.1f}s  [stop_reason={stop_reason}]")
    sf.write(output_path, frames, config.SAMPLE_RATE)
    return output_path
